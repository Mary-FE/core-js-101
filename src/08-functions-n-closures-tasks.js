...
function retry(func, attempts) {
  return () => {
    ...
    while (trying <= attempts) {
      try { return func(); } catch (error) { trying += 1; }
    }
...
function logger(func, logFunc) {
  return (...rest) => {
    const income = JSON.stringify(rest).slice(1, -1);

    logFunc(`${func.name}(${income}) starts`);
    const result = func(...rest);
    logFunc(`${func.name}(${income}) ends`);
